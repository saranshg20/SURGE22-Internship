...
void AudioEngine::writeToFile(const char* filePath, int offset) {

    LOGD(TAG, "writeToFile(): ");
//    mSoundRecording.setOffsetValue(offset);
//    mSoundRecording.initiateWritingToFile(filePath, mOutputChannelCount, static_cast<int32_t>(mSampleRate*0.47));
//    mSoundRecording.initiateWritingToFile(filePath, mOutputChannelCount, mSampleRate);
    mSoundRecording.initiateWritingToFile(filePath, mOutputChannelCount, static_cast<int32_t>(mSampleRate));
}
...
//        assert(mPlaybackStream->getSampleRate() == mSampleRate);
        ...
        LOGV("mFramesPerBurst: %s", std::to_string(mFramesPerBurst*2).c_str());

        // Set the buffer size to the burst size - this will give us the minimum possible latency
        mPlaybackStream->setBufferSizeInFrames(mFramesPerBurst*2);
...
//        assert(mPlaybackStream->getSampleRate() == mSampleRate);
        ...
        LOGV(TAG, std::to_string(mFramesPerBurst*2).c_str());
...
//        audioStreamAAudio = dynamic_cast<oboe::AudioStreamAAudio *>(stream);
...
void AudioEngine::setOffsetValue(int i) {
    mSoundRecording.setOffsetValue(i);
}

void AudioEngine::setGainFactor(double d) {
    mSoundRecording.setGainFactor(d);
}

void AudioEngine::setSampleRateFactor(double d) {
    mSoundRecording.setSampleRateFactor(d);
}
